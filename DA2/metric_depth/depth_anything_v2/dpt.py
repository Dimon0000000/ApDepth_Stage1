...
class DepthAnythingV2(nn.Module):
    ...
    def infer_batch(self, images: torch.Tensor):
        ...
        # depth_vis = depth_vis - depth_vis.min(dim=-1, keepdim=True)[0].min(dim=-2, keepdim=True)[0]
        # depth_vis = depth_vis / (depth_vis.max(dim=-1, keepdim=True)[0].max(dim=-2, keepdim=True)[0] + 1e-6)

        # # 只保存 batch 中前几张，防止一次存太多
        # max_save = min(B, 4)
        # for i in range(max_save):
        #     save_path = os.path.join(save_dir, f"depth_{i}.png")
        #     vutils.save_image(
        #         depth_vis[i],  # [1,H,W]
        #         save_path,
        #         normalize=False
